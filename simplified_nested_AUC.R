# Delta AUC Nested Model - Simplified Version
library("e1071")
library("MASS")
...
for(i in 1:R)
{
  ...
  for(j in 1:B)
  {
    ...
    # Fit and evaluate all models using the unified function
    result_lda = fit_and_evaluate_model("lda", df_train, df_test, y_test)
    auc.lda.test.full[i,j] = result_lda$auc_full
    auc.lda.test.redu[i,j] = result_lda$auc_redu
    delong.lower.split.lda[i,j] = result_lda$delong_lower
    delong.upper.split.lda[i,j] = result_lda$delong_upper

    result_logi = fit_and_evaluate_model("logistic", df_train, df_test, y_test)
    auc.logi.test.full[i,j] = result_logi$auc_full
    auc.logi.test.redu[i,j] = result_logi$auc_redu
    delong.lower.split.logi[i,j] = result_logi$delong_lower
    delong.upper.split.logi[i,j] = result_logi$delong_upper

    result_svm = fit_and_evaluate_model("svm", df_train, df_test, y_test)
    auc.svm.test.full[i,j] = result_svm$auc_full
    auc.svm.test.redu[i,j] = result_svm$auc_redu
    delong.lower.split.svm[i,j] = result_svm$delong_lower
    delong.upper.split.svm[i,j] = result_svm$delong_upper

    result_xgb = fit_and_evaluate_model("xgboost", df_train, df_test, y_test, nrounds = 100)
    auc.xgb.test.full[i,j] = result_xgb$auc_full
    auc.xgb.test.redu[i,j] = result_xgb$auc_redu
    delong.lower.split.xgb[i,j] = result_xgb$delong_lower
    delong.upper.split.xgb[i,j] = result_xgb$delong_upper
  }
}
